...
# Adjust Medaka parameters for less stringent error correction
medaka_consensus -i ".${output_s}/${id}_16s_bin.fasta" -d "${fasta_16s_database}" -o ".${bb_p}/${id}_16s_medaka_1" -m "${medaka_model}" -t "${para_cores}" -b "${para_RAM}" -f >> ".${bb_p}/medaka_stdout_log.txt" 2>&1

echo "Iteration 1 - 18s" >> ".${bb_p}/medaka_stdout_log.txt"
medaka_consensus -i ".${output_s}/${id}_18s_bin.fasta" -d "${fasta_18s_database}" -o ".${bb_p}/${id}_18s_medaka_1" -m "${medaka_model}" -t "${para_cores}" -b "${para_RAM}" -f >> ".${bb_p}/medaka_stdout_log.txt" 2>&1
...
medaka_consensus -i ".${bb_p}/${id}_16s_medaka_1/consensus.fasta" -d "${fasta_16s_database}" -o ".${bb_p}/${id}_16s_medaka_2" -m "${medaka_model}" -t "${para_cores}" -b "${para_RAM}" -f >> ".${bb_p}/medaka_stdout_log.txt" 2>&1

echo "Iteration 2 - 18s" >> ".${bb_p}/medaka_stdout_log.txt"
medaka_consensus -i ".${bb_p}/${id}_18s_medaka_1/consensus.fasta" -d "${fasta_18s_database}" -o ".${bb_p}/${id}_18s_medaka_2" -m "${medaka_model}" -t "${para_cores}" -b "${para_RAM}" -f >> ".${bb_p}/medaka_stdout_log.txt" 2>&1
...
## 18S
echo "8,Importing and classifying ${seq_count_medaka_18s_2} ($percentage_retained_18s_medaka_it2% retained) polished 18S amplicons in QIIME2 ($default_database_name $default_database_version)" > $progress_file
echo "8,Importing and classifying ${seq_count_medaka_18s_2} ($percentage_retained_18s_medaka_it2% retained) polished 18S amplicons in QIIME2 ($default_database_name $default_database_version)" >> ${log}
...
stdbuf -oL kraken2-build --db ".${output_p}/${id}_16s_${run_time}-CON" --kmer-len 35 --minimizer-len 31 --minimizer-spaces 6 --threads ${cores} --build >> ${log} 2>&1 || { sleep 5; echo "###failed here: Kraken 16s database creation###" >> "${log}"; exit 1; }
...
stdbuf -oL kraken2-build --db ".${output_e}/${id}_18s_${run_time}-CON" --kmer-len 35 --minimizer-len 31 --minimizer-spaces 6 --threads ${cores} --build >> ${log} 2>&1 || { sleep 5; echo "###failed here: Kraken 18s database creation###" >> "${log}"; exit 1; }
...
bracken -d ".${output_e}/${id}_18s_${run_time}-CON" -i ".${output_e}/kraken_report_18s.txt" -o ".${output_e}/braken_18s.tsv" -r "$len_18s" -l S -t ${cores} >> ${log} 2>&1 || { sleep 5; echo "###failed here: Bracken matching raw and 18s con data ###" >> ${log}; exit 1; }
...
# Remove excess from bracken outputs and re-name headers
