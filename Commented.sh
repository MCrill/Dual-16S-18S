...
# Check the Kraken report before filtering
if [ ! -s ".${output_p}/kraken_report_16s.txt" ]; then
  echo "###Kraken report is empty. No reads classified###" >> ${log}
  exit 1
fi

# Filter out problematic species from the Kraken2 report less aggressively
...
# Check if the filtered report is empty
if [ ! -s ".${output_p}/kraken_report_16s_filtered.txt" ]; then
  echo "###Filtered Kraken report is empty. No reads found after filtering###" >> ${log}
  exit 1
fi

# Recalculate 16s abundances with filtered report
bracken-build -l 500 -d ".${output_p}/${id}_16s_${run_time}-CON" -t ${cores} >> ${log} 2>&1
bracken -d ".${output_p}/${id}_16s_${run_time}-CON" -i ".${output_p}/kraken_report_16s_filtered.txt" -o ".${output_p}/braken_16s.tsv" -r 500 -l S -t ${cores} >> ${log} 2>&1 || { sleep 5; echo "###failed here: Bracken matching raw and 16s con data ###" >> "${log}"; exit 1; }
...
# Check the Kraken report before filtering
if [ ! -s ".${output_e}/kraken_report_18s.txt" ]; then
  echo "###Kraken report is empty. No reads classified###" >> ${log}
  exit 1
fi
...
# Check if the filtered report is empty
if [ ! -s ".${output_e}/kraken_report_18s_filtered.txt" ]; then
  echo "###Filtered Kraken report is empty. No reads found after filtering###" >> ${log}
  exit 1
fi

# Recalculate 18s abundances with filtered report
bracken-build -l 500 -d ".${output_e}/${id}_18s_${run_time}-CON" -t ${cores} >> ${log} 2>&1
bracken -d ".${output_e}/${id}_18s_${run_time}-CON" -i ".${output_e}/kraken_report_18s_filtered.txt" -o ".${output_e}/braken_18s.tsv" -r 500 -l S -t ${cores} >> ${log} 2>&1 || { sleep 5; echo "###failed here: Bracken matching raw and 18s con data ###" >> "${log}"; exit 1; }
